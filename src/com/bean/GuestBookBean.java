...
public class GuestBookBean {
	...
	private int count = -1;
	private int qq = 0;
	private String sql="select count(*) from fz_guestbook where ifhide='1'";
	private String sql2="select * from fz_guestbook where ifhide='1' order by addtime desc ";
	...
	//private String sql4="select * from fz_guestbook order by addtime desc ";
    public int getMessageCountM(String stime,String etime) { //得到信息总数
    	String sql3=null;
       if(stime.equals("0")){//屏蔽的
    	   sql3="select count(*) from fz_guestbook where ifhide='0'";
       }   
       else if(stime.equals("1")){//显示的
    	   sql3="select count(*) from fz_guestbook where ifhide='1'";
       }
       else if(stime.equals("2")){//所有的
    	   sql3="select count(*) from fz_guestbook ";
       }
       else{
    	   sql3="select count(*) from fz_guestbook where addtime between '"+stime+"' and '"+etime+" 23:59:59'";
       }
       DBO dbo=new DBO();
       dbo.open();
        try { 
    ...
    public List getMessageM(int page,String stime,String etime) { //得到每页要显示的信息
    	String sql4=null;
        if(stime.equals("0")){//屏蔽的
        	sql4="select * from fz_guestbook where ifhide='0'";
        }   
        else if(stime.equals("1")){//显示的
        	sql4="select * from fz_guestbook where ifhide='1'";
        }
        else if(stime.equals("2")){//所有的
        	sql4="select * from fz_guestbook order by addtime desc ";
        }
        else{
        	sql4="select * from fz_guestbook where addtime between '"+stime+"' and '"+etime+" 23:59:59'";
        }
        DBO dbo=new DBO();
        dbo.open();
        List list = new ArrayList();
    ...
    public int hideGuestBook(int id){
    	String sql = "select ifhide from fz_guestbook where id='"+id+"'  ";
    	String sql2 = "update fz_guestbook set ifhide='0' where id ='"+id+"'";
    	String sql3 = "update fz_guestbook set ifhide='1' where id ='"+id+"'";
    	DBO dbo = new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);
	...
	public int addGuestBook(String nickname,String pic,String email,String qq,String weburl,String blogurl,String expressions,String content,String ip,int ifhide){
		String sql = "insert into fz_guestbook ( nickname, pic, email, qq, weburl, blogurl, expressions, content,addtime,ip,replay,ifhide)" +
	...
	public String getGuestBook(int id){
		String sql = "select content from fz_guestbook where id='"+id+"' ";
		DBO dbo=new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);
	...
	public String getReplay(int id){
		String sql = "select replay from fz_replay where mid='"+id+"' ";
		DBO dbo=new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);
...
//	查询回复内容
	public List getReplayInfo(int id){ 
		String sql = "select * from fz_replay where mid='"+id+"' ";
		DBO dbo=new DBO();
		list = new ArrayList();
		dbo.open();
		try{
	...
	public int delGuestBook(int id[]){
		...
		try{
			for(int i = 0;i<id.length;i++){
				dbo.executeUpdate("delete from  fz_guestbook  where  id = '"+id[i]+"'");	
				dbo.executeUpdate("delete from  fz_replay  where  mid = '"+id[i]+"'");	
			}
			return Constant.SUCCESS;
		}catch(Exception e){
			e.printStackTrace();
	...
	public int reGuestBook(int mid ,String replay,String replayer){
		String sql = "insert into fz_replay (mid,replay,replayer,replaytime)" +
				" values ('"+mid+"','"+replay+"','"+replayer+"','"+date+"') ";
		String sql2 = "update fz_guestbook set replay='1' where id='"+mid+"' ";
		DBO dbo = new DBO();
		dbo.open();
		try{
			int i = dbo.executeUpdate(sql);
	...
	public int upReplay(int mid ,String replay){
		String sql = "update fz_replay set replay='"+replay+"',replaytime='"+date+"' where mid='"+mid+"' ";
		DBO dbo = new DBO();
		dbo.open();
		try{
			int i = dbo.executeUpdate(sql);
